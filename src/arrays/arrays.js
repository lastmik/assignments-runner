export function mapTo(array, property) {
  ...
  return array.map(function(elem){ return array.indexOf(elem)});
...
export function reduceTo(array, propertys) {
  ...
  {
    return array.reduce((accumulator, currentValue)=> accumulator+ currentValue[propertys],0);
  }
  else{
    propertys.forEach(property =>{
      resultArray.push(array.reduce((accumulator, currentValue)=> accumulator+ currentValue[property],0))
    });
    return resultArray;
  }
...
export function sort(array, filter) {
  ...
  function sortArray(a,b){
    ...
    }else if(filter ==='age')
    {
      ...
      return 2 * (a[filter[0]] > b[filter[0]] ? 1 : a[filter[0]] < b[filter[0]] ? -1 : 0) + 1 * (a[filter[1]] > b[filter[1]] ? 1 : a[filter[1]] < b[filter[1]] ? -1 : 0);
    }
    else{
      if(filter[1].order ==='desc'){
        return 2 * (a[filter[0]] > b[filter[0]] ? 1 : a[filter[0]] < b[filter[0]] ? -1 : 0) + 1 * (b[filter[1].field] > a[filter[1].field] ? 1 : b[filter[1].field] < a[filter[1].field] ? -1 : 0);
      }else{
        return 2 * (a[filter[0]] > b[filter[0]] ? 1 : a[filter[0]] < b[filter[0]] ? -1 : 0) + 1 * (a[filter[1].field] > b[filter[1].field] ? 1 : a[filter[1].field] < b[filter[1].field] ? -1 : 0);
      }
    }
...
export function complex(array, operations ) {
  ...
  function filtering(properties){
    array = array.filter(elem => properties.callback(elem[properties.property]));
  };
  ...
  function reducing(properties){
    result = array.reduce((accumulator, currentValue)=> accumulator+ currentValue[properties.property],0);
  };
